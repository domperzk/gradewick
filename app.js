...
let _activeTimetableTab = {};

function buildTimetablesWrapper(yr) {
  const activeTab = _activeTimetableTab[yr.id] || 'university';

  const toggleHtml = `
    <div style="display:flex;gap:8px;margin-bottom:20px;border-bottom:1.5px solid var(--b1);padding-bottom:16px">
      <button class="btn ${activeTab === 'university' ? 'btn-primary' : 'btn-ghost'}" onclick="switchInternalTimetable('${yr.id}', 'university')">University Timetable</button>
      <button class="btn ${activeTab === 'exam' ? 'btn-primary' : 'btn-ghost'}" onclick="switchInternalTimetable('${yr.id}', 'exam')">Exam Timetable</button>
    </div>
  `;

  const contentHtml = activeTab === 'university'
    ? (typeof buildSchoolTimetable === 'function' ? buildSchoolTimetable(yr) : '<div class="tt-empty">Timetable module not loaded.</div>')
    : buildTimetable(yr);

  return toggleHtml + contentHtml;
}

function switchInternalTimetable(yid, tab) {
  _activeTimetableTab[yid] = tab;
  const pane = document.getElementById(`sp-${yid}-timetables`);
  if (pane) pane.innerHTML = buildTimetablesWrapper(getYear(yid));
}
...
function renderSidebarNav(yid) {
  ...
  const st  = _yearSubtabs[yid] || APP.lastTab?.[yid] || 'timetables'; // Make it default
  ...
  const tabs = [
    ...
    { id: 'timetables', icon: `<svg viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="17" rx="2"/><path d="M8 2v4M16 2v4M3 10h18M8 14h.01M12 14h.01M16 14h.01M8 18h.01M12 18h.01M16 18h.01"/></svg>` },
...
function renderYearPane(yid) {
  ...
  pane.innerHTML=`
    ...
    <div class="subpane ${st==='timetables'?'active':''}" id="sp-${yid}-timetables">${buildTimetablesWrapper(yr)}</div>
...
function onMark(yid, compId, el) {
  ...
  const ttp=document.getElementById(`sp-${yid}-timetables`);
  ...
  if(dsp) dsp.innerHTML=buildDashboard(yr);
  if(ttp) ttp.innerHTML=buildTimetablesWrapper(yr);
