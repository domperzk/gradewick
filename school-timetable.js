...
//
// What this adds:
//  • A new "School Timetable" tab in every year's sidebar navigation
...
//  • Week-grid view  (Mon–Fri, 08:00–20:00, proportional height blocks)
//  • List view       (chronological, grouped by day)
//  • "Today" button, prev/next week navigation
...
//  • Import overlay accessible from sidebar footer as well as the tab itself
// ─────────────────────────────────────────────────────────────────────────────

// ── Constants ──────────────────────────────────────────────────────────────
const TT_STORAGE_KEY   = 'gradewick_timetable_v1';
const TT_VIEW_KEY      = 'gradewick_timetable_view';
const TT_WEEK_KEY      = 'gradewick_timetable_week'; // ISO string of Mon of current week
...
// ── State ──────────────────────────────────────────────────────────────────
let _ttView       = 'week';   // 'week' | 'list'
let _ttWeekStart  = null;     // Date object — Monday of displayed week
...
// ── Week nav state initialisation ─────────────────────────────────────────

function ttEnsureWeekStart() {
  if (_ttWeekStart) return;
  const saved = localStorage.getItem(TT_WEEK_KEY);
  _ttWeekStart = saved ? ttMondayOf(new Date(saved)) : ttMondayOf(new Date());
}

function ttSaveWeek() {
  if (_ttWeekStart) localStorage.setItem(TT_WEEK_KEY, _ttWeekStart.toISOString());
}
...
function buildSchoolTimetable(yr) {
  ...
  const viewToggle = `
    <div class="tt-view-toggle" style="display:flex;align-items:center;gap:8px;margin-bottom:20px;flex-wrap:wrap">
      <button class="tt-flt ${_ttView==='week'?'active':''}" onclick="ttSetView('${yr.id}','week')">📅 Week</button>
      <button class="tt-flt ${_ttView==='list'?'active':''}" onclick="ttSetView('${yr.id}','list')">📋 List</button>
  ...
  const content = _ttView === 'week'
    ? ttBuildWeekView(yr.id, events)
    : ttBuildListView(yr.id, events);
...
function ttBuildWeekView(yid, events) {
  ...
  const nav = `
    <div class="tt-week-nav">
      <button class="icon-btn" onclick="ttNavWeek('${yid}',-1)" title="Previous week">◀</button>
      <div class="tt-week-label">${weekLabel}</div>
      <button class="icon-btn" onclick="ttNavWeek('${yid}',1)" title="Next week">▶</button>
...
function ttBuildListView(yid, events) {
  if (!events.length) return '<div class="tt-empty">No events.</div>';

  // Sort all events chronologically
  const sorted = [...events].filter(ev => ev.start).sort((a, b) => a.start - b.start);
...
function ttNavWeek(yid, delta) {
  ttEnsureWeekStart();
  _ttWeekStart = ttAddDays(_ttWeekStart, delta * 7);
  ttSaveWeek();
  ttRenderPane(yid);
}

function ttGoToday(yid) {
  _ttWeekStart = ttMondayOf(new Date());
  ttSaveWeek();
  ttRenderPane(yid);
}
 ...
 *  3. A "School Timetable" button in each year's sidebar footer
 *
...
function ttInjectDOM() {
  ...
  if (footer) {
    ...
    // Insert before the Settings button (last .nav-btn before sidebar-credit)
    btn.onclick = () => {
      const yr = activeYear();
      if (yr) {
        switchSubtab(yr.id, 'schooltimetable');
        closeSidebar();
      }
    };
    btn.innerHTML = `<div class="nav-btn-left"><span class="nav-icon">🗓</span> <span class="nav-lbl">School Timetable</span></div>`;
    // Insert as first footer button
...
(function ttInit() {
  ...
  if (savedView === 'list' || savedView === 'week') _ttView = savedView;
